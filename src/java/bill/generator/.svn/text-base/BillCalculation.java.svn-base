...
public class BillCalculation {
        ...
        try {
            ...
            if (rs != null) {
                while (rs.next()) {
                    ...
                    if (cbl_per_fr.after(new GregorianCalendar(2006, 2, 31))) {
                        paidcss = rs.getDouble(7);
                    } else {
                        paidcss = 0;
                    }
    ...
    public ArrayList<ArrayList> calbillCl(String cons_no, Calendar billdate, Calendar due_dt, String sec, String xmlpath, Double ttor_arr) {
        ...
        int flag4 = 0;
        double advbillamt = 0;
        double remyearbillamt = 0;
        double billamt1 = 0;
        // int bflag=0;
        prepay_date = null;
        double curbillamt;
        double yearbillamt = 0;
        double intbillamt = 0;

        //flag to check if rate is 0 for some bill cycle for a consumer whose bill is being calculated
        int rate_flag = 0;
        ...
        try {
            ...
            //tto_arr hold the value of arrer when connection status is changed to from temp to regular
            if (ttor_arr > 0) {
                arrear = ttor_arr;
                arrear1 = ttor_arr;
            }
            ...
            for (int a = 0; a < al.size(); a++) {
                ...
                ///////
                if (advbillamt > 0) {
                    advbillamt = advbillamt - bill_amt;
                    if (advbillamt > 0) {
                        bill_amt = 0.0;
                    } else {
                        bill_amt = Math.abs(advbillamt);
                        advbillamt = 0.0;
                    }
                }
                ...
                //remyearbillamt=yearbillamt-(billamt1+curbillamt);
                remyearbillamt = rate * intmonth - curbillamt;
                ...
                if (bl_per_fr.before(new GregorianCalendar(2002, 3, 1))) {
                    if (pay_date == null) //if pay_date is null
                    {
                        if (prepay_date != null) {
                            ...
                            if (prepay_date.before(bl_per_to) || ((prepay_date.get(Calendar.YEAR) == bl_per_to.get(Calendar.YEAR))
                                    && (prepay_date.get(Calendar.MONTH) == bl_per_to.get(Calendar.MONTH))
                                    && (prepay_date.get(Calendar.DATE) == bl_per_to.get(Calendar.DATE)))) {
                                prepay_date = null;
                            }
                        } else {
                            intmonth = yearDiff(bl_per_fr, bl_per_to);
                    ...
                    } else // if pay_date is not null
                    {
                        ...
                        if (pay_date.after(bl_per_to)) {
                            ...
                            if (prepay_date != null && prepay_date.before(pay_date)) {
                                ...
                                if (prepay_date.before(pay_date) || ((prepay_date.get(Calendar.YEAR) == pay_date.get(Calendar.YEAR))
                                        && (prepay_date.get(Calendar.MONTH) == pay_date.get(Calendar.MONTH))
                                        && (prepay_date.get(Calendar.DATE) == pay_date.get(Calendar.DATE)))) {
                                    prepay_date = null;
                                }
                            } else {
                                intmonth = 0;
                    ...
                    if (pay_date == null) {
                        ...
                        if (pay_date.after(bl_per_to)) {
                            ...
                            if ((bl_per_to.get(Calendar.MONTH) != 2)) {
                                ...
                                if (billamt > 0) //calculate interest on previous unpaid billamt
                                {
                                    ...
                                    intbillamt = intbillamt + interest;
                                    arrear = arrear + interest;
                                    totinterest = totinterest + interest;
                                }
                                ...
                                paid_amt = bill_amt - paid_amt;
                                if (paid_amt < 0) {
                                    paid_amt = Math.abs(paid_amt);
                                    paid_amt = intbillamt - paid_amt;
                                    if (paid_amt < 0) {
                                        paid_amt = Math.abs(paid_amt);
                                        billamt = billamt - intbillamt;
                                        arrear = arrear - intbillamt;
                                        intbillamt = 0;
                                    } else {
                                        billamt = billamt - (intbillamt - paid_amt);
                                        arrear = arrear - (intbillamt - paid_amt);
                                        intbillamt = paid_amt;
                                        paid_amt = 0;
                                    }
                                    if (paid_amt > 0) {
                                        paid_amt = (billamt - bill_amt) - paid_amt;
                                        if (paid_amt < 0) {
                                            paid_amt = Math.abs(paid_amt);
                                            arrear = arrear - (billamt - bill_amt);
                                            billamt = 0;
                                        } else {
                                            arrear = arrear - ((billamt - bill_amt) - paid_amt);
                                            billamt = paid_amt + bill_amt;
                                            paid_amt = 0;
                                        }
                                    }
                                    bill_amt = 0.0;
                                ...
                                remyearbillamt = remyearbillamt + bill_amt;

                                if (paid_amt > 0) {

                                    if (remyearbillamt <= paid_amt + advbillamt) {
                                        paid_amt = advbillamt + paid_amt - remyearbillamt;
                                        advbillamt = remyearbillamt;
                                    } else {
                                        advbillamt = advbillamt + paid_amt;
                                        paid_amt = 0;
                                    }
                            ...
                            {
                                /////////////////////////////////////
                                pramt = arrear + bill_amt * bilcycle - rebate - totinterest;
                                pdamt = pramt - paid_amt;
                                if (pdamt < 0) {
                                    totinterest = totinterest - Math.abs(pdamt);
                                }
                                ...
                                // remyearbillamt=rate*12-(billamt1+curbillamt);
                                ...
                                if (paid_amt < 0) {
                                    ...
                                    if (paid_amt > 0) {
                                        paid_amt = (billamt) - paid_amt;
                                        if (paid_amt < 0) {
                                            paid_amt = Math.abs(paid_amt);
                                            arrear = arrear - (billamt);
                                            billamt = 0;
                                        } else {
                                            arrear = arrear - ((billamt) - paid_amt);
                                            billamt = paid_amt;
                                            paid_amt = 0;
                                        }
                                    }
                ...
                if (pay_date != null) {
                    if (pay_date.after(bl_per_to)) {
                        ...
                        prepay_date.set(pay_date.get(Calendar.YEAR), pay_date.get(Calendar.MONTH), pay_date.get(Calendar.DATE));
                    }
                }
