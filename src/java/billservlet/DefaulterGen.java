 ...
 * @author smp
 */
public class DefaulterGen extends HttpServlet {
...
//            }
            ...
            switch (pdf.getPercentage()) {
                ...
                default:
                    ...
                    alsec = pdf.calAl();
                    context = getServletContext();
                    String path = context.getRealPath("") + "/resources/xmlreports/defaulter_" + sec + ".xml";
        ...
        DataSource dataSource=(DataSource)initialContext.lookup("OnlineJal");
        con=dataSource.getConnection();
                        path = context.getRealPath("") + "/resources/xmlreports/defaulter_" + sec.trim() + ".xml";
               ...
               rs=pst.executeQuery(sql);
                 ...
                 content_id=content_id+1;}
                     ...
                     content_id=new Long(1);
                        ...
                        pst.setString(5, "defaulter_report");
...
//-----------------------------------------------------------------------------------------//
    ...
    private void isBusy(MyPdf pdf, ServletOutputStream stream, int totcons, String bill_date, String sec, String path, String userid)
        ...
        stream.print("<p>Ledger for " + pdf.cons_no + " is being calculated." + String.valueOf(totcons - pdf.getPercentage()));
        stream.print("&nbsp;consumers are left.<br>\nPlease Wait while this page refreshes automatically (every 5 seconds)</p></div><form action=\"/OnlineJal_Example/CancelDeflt\">"
    ...
    public void isFinished(ServletOutputStream stream, String bill_date, String sec, String path, String userid) throws IOException {
        ...
        stream.print("<br/><br/>The document is finished:&nbsp;&nbsp;&nbsp;&nbsp;</div><form method=\"POST\" action=\"/OnlineJal_Example/DefaulterGen\">"
                + "<input type=\"hidden\" name=\"path\" value=\"" + path + "\">"
                + "<input type=\"hidden\" name=\"sec\" value=\"" + sec + "\"><input type=\"hidden\" name=\"billdate\" value=\"" + bill_date + "\">"
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        document1.open();
        float[] widths1 = {1f, 2f, 1f, 1f, 1f, 1f};
        ...
        cell = new PdfPCell(new Paragraph("DEFAULTER'S LIST FOR SECTOR - " + sec.trim() + "(ARREAR UPTO-" + billdate + ")"));
        cell.setBorder(Rectangle.NO_BORDER);
        cell.setColspan(6);
        cell.setHorizontalAlignment(Element.ALIGN_CENTER);
        ...
        try {
            ...
            for (int s = 0; s < consumer.getLength(); s++) {
                ...
                if (cons.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    cell = new PdfPCell(new Paragraph(((Node) principaltext.item(0)).getNodeValue().trim().replace("-0.0", "0.0")));
                    cell.setBorder(Rectangle.NO_BORDER);
                    cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                    table.addCell(cell);
                    ...
                    cell = new PdfPCell(new Paragraph(((Node) interesttext.item(0)).getNodeValue().trim()));
                    cell.setBorder(Rectangle.NO_BORDER);
                    cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                    table.addCell(cell);
                    ...
                    cell = new PdfPCell(new Paragraph(((Node) arreartext.item(0)).getNodeValue().trim()));
                    cell.setBorder(Rectangle.NO_BORDER);
                    cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                    table.addCell(cell);
            ...
            response.sendRedirect("jsppages/manager/defaulters_creditors.jsp");
            return;
